...
- (void)viewDidLoad
{
    ...
    zen180_IAP_Helper.relaxedMindVC = self;
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.RelaxedMind_IAP"] == YES || [[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.Pack_IAP"] == YES)
    {
        if ([[NSUserDefaults standardUserDefaults]boolForKey:@"hasStartedDownloadingRelaxedMind"] == YES)
        {
            [self showUnlockButton];
            viewHasAlreadyLoadedOnce = YES;
        }
        else if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"relaxedMindDownloadStatus"]  isEqual: @"complete"])
        {
...
{
    appDelegate.currentView = @"relaxedMind";
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (fiveMinuteRelaxedMindExists)
        [mp3Array addObject:@"1"];
    if (fifteenMinuteRelaxedMindExists)
        [mp3Array addObject:@"1"];
    if (thirtyMinuteRelaxedMindExists)
        [mp3Array addObject:@"1"];
    ...
    }else if (viewHasAlreadyLoadedOnce == NO)
    {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.RelaxedMind_IAP"] == YES || [[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.Pack_IAP"] == YES)
        {
            ...
            else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"hasStartedDownloadingRelaxedMind"] == YES)
            {
...
- (void)productPurchased:(NSNotification *)notification
{
    ...
    [productsArray enumerateObjectsUsingBlock:^(SKProduct *product, NSUInteger idx, BOOL *stop) {
        if ([product.productIdentifier isEqualToString:@"com.urbndna.RelaxedMind_IAP"] || [product.productIdentifier isEqualToString:@"com.urbndna.Pack_IAP"])
        {
...
- (IBAction)buySingleTrack:(id)sender
{
    ...
    {
        if ([product.productIdentifier isEqualToString:@"com.urbndna.RelaxedMind_IAP"])
        {
    ...
    [[Zen180_IAP_Helper sharedInstance] buyProduct:product];
        ...
        [userDefaults setBool:YES forKey:@"hasStartedDownloadingRelaxedMind"];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    {
        [self setupAudioPlayer:@"Relaxed Mind 5 - Delta"];
    ...
    {
        [self setupAudioPlayer:@"Relaxed Mind 15 - Delta"];
        [self.playButton setImage:[UIImage imageNamed:@"play-64.png"] forState:UIControlStateNormal];
    }
    
    else
        [self setupAudioPlayer:@"Relaxed Mind 30 - Delta"];
    [self.playButton setImage:[UIImage imageNamed:@"play-64.png"] forState:UIControlStateNormal];
...
- (void)redownload
{
    if(!fiveMinuteRelaxedMindExists && fifteenMinuteRelaxedMindExists && thirtyMinuteRelaxedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fifteenMinuteDownload && fiveMinuteRelaxedMindExists && thirtyMinuteRelaxedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!thirtyMinuteRelaxedMindExists && fiveMinuteRelaxedMindExists && fifteenMinuteRelaxedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fiveMinuteRelaxedMindExists && !fifteenMinuteRelaxedMindExists && thirtyMinuteRelaxedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fifteenMinuteRelaxedMindExists && !thirtyMinuteRelaxedMindExists && fiveMinuteRelaxedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fiveMinuteRelaxedMindExists && !thirtyMinuteRelaxedMindExists && fifteenMinuteRelaxedMindExists)
    {
        buyPackView.hidden = NO;
...
- (void)downloadFiveMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Relaxed%20Mind%205%20-%20Delta.mp3"];
...
- (void)downloadFifteenMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Relaxed%20Mind%2015%20-%20Delta.mp3"];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
...
- (void)downloadThirtyMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Relaxed%20Mind%2030%20-%20Delta.mp3"];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        NSProgress *progress;
        NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:&progress destination:^NSURL *(NSURL *targetPath, NSURLResponse *response){
            NSURL *documentsDirectoryURL = [[NSFileManager defaultManager] URLForDirectory:NSDocumentDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:NO error:nil];
...
- (void)successAlert
{
    successAlert = [[UIAlertView alloc] initWithTitle:@"Message" message:@"Relaxed Mind- Download Successful" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
    [successAlert show];
    [userDefaults setValue:@"complete" forKey:@"relaxedMindDownloadStatus"];
    [userDefaults synchronize];
    buyPackView.hidden = YES;
    lockedButtonOutlet.hidden = YES;
    timeOptionsPickerView.hidden = NO;
...
- (void) checkIfFileIsPresent
{
    ...
    NSString *fiveMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Relaxed Mind 5 - Delta.mp3"];
    NSString *fifteenMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Relaxed Mind 15 - Delta.mp3"];
    NSString *thirtyMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Relaxed Mind 30 - Delta.mp3"];
    
    fiveMinuteRelaxedMindExists = [[NSFileManager defaultManager] fileExistsAtPath:fiveMinuteMp3];
    
    fifteenMinuteRelaxedMindExists = [[NSFileManager defaultManager] fileExistsAtPath:fifteenMinuteMp3];
    
    thirtyMinuteRelaxedMindExists = [[NSFileManager defaultManager] fileExistsAtPath:thirtyMinuteMp3];

    if (fiveMinuteRelaxedMindExists == YES && fifteenMinuteRelaxedMindExists == YES && thirtyMinuteRelaxedMindExists == YES)
    {
...
- (void)ifAllDoneRemoveDownloadWindow
{
    ...
    {
        [userDefaults setValue:@"complete" forKey:@"relaxedMindDownloadStatus"];
        [userDefaults setBool:NO forKey:@"hasStartedDownloadingRelaxedMind"];
        [userDefaults synchronize];
        buyPackView.hidden = YES;
        lockedButtonOutlet.hidden = YES;
        timeOptionsPickerView.hidden = NO;
