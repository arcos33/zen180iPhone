...
- (void)viewDidLoad
{
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.AwakenedMind_IAP"] == YES || [[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.Pack_IAP"] == YES)
    {
        if ([[NSUserDefaults standardUserDefaults]boolForKey:@"hasStartedDownloadingAwakendMind"] == YES)
        {
            [self showUnlockButton];
            viewHasAlreadyLoadedOnce = YES;
        }
        else if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"awakenedMindDownloadStatus"]  isEqual: @"complete"])
        {
...
{
    appDelegate.currentView = @"awakenedMind";
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (fiveMinuteAwakenedMindExists)
        [mp3Array addObject:@"1"];
    if (fifteenMinuteAwakenedMindExists)
        [mp3Array addObject:@"1"];
    if (thirtyMinuteAwakenedMindExists)
        [mp3Array addObject:@"1"];
    ...
    }else if (viewHasAlreadyLoadedOnce == NO)
    {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.AwakenedMind_IAP"] == YES || [[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.Pack_IAP"] == YES)
        {
            ...
            else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"hasStartedDownloadingAwakendMind"] == YES)
            {
...
- (void)productPurchased:(NSNotification *)notification
{
    ...
    [productsArray enumerateObjectsUsingBlock:^(SKProduct *product, NSUInteger idx, BOOL *stop) {
        if ([product.productIdentifier isEqualToString:@"com.urbndna.AwakenedMind_IAP"] || [product.productIdentifier isEqualToString:@"com.urbndna.Pack_IAP"])
        {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    {
        [self setupAudioPlayer:@"Awakened Mind 5 - Beta"];
    ...
    {
        [self setupAudioPlayer:@"Awakened Mind 15 - Beta"];
        [self.playButton setImage:[UIImage imageNamed:@"play-64.png"] forState:UIControlStateNormal];
    }
    
    else
        [self setupAudioPlayer:@"Awakened Mind 30 - Beta"];
    [self.playButton setImage:[UIImage imageNamed:@"play-64.png"] forState:UIControlStateNormal];
...
- (IBAction)buySingleTrack:(id)sender
{
    ...
    {
        if ([product.productIdentifier isEqualToString:@"com.urbndna.AwakenedMind_IAP"])
        {
    ...
    appDelegate.hasStartedDownloadingAwakenedMind = [NSNumber numberWithInt:1];
    NSLog(@"AppDelegate hasStarted = %@", appDelegate.hasStartedDownloadingAwakenedMind);
    [userDefaults setBool:YES forKey:@"hasStartedDownloadingAwakendMind"];
...
- (void)redownload
{
    if(!fiveMinuteAwakenedMindExists && fifteenMinuteAwakenedMindExists && thirtyMinuteAwakenedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fifteenMinuteDownload && fiveMinuteAwakenedMindExists && thirtyMinuteAwakenedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!thirtyMinuteAwakenedMindExists && fiveMinuteAwakenedMindExists && fifteenMinuteAwakenedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fiveMinuteAwakenedMindExists && !fifteenMinuteAwakenedMindExists && thirtyMinuteAwakenedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fifteenMinuteAwakenedMindExists && !thirtyMinuteAwakenedMindExists && fiveMinuteAwakenedMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fiveMinuteAwakenedMindExists && !thirtyMinuteAwakenedMindExists && fifteenMinuteAwakenedMindExists)
    {
        buyPackView.hidden = NO;
...
- (void)downloadFiveMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Awakened%20Mind%205%20-%20Beta.mp3"];
...
- (void)downloadFifteenMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Awakened%20Mind%2015%20-%20Beta.mp3"];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
...
- (void)downloadThirtyMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Awakened%20Mind%2030%20-%20Beta.mp3"];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        NSProgress *progress;
        NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:&progress destination:^NSURL *(NSURL *targetPath, NSURLResponse *response){
            NSURL *documentsDirectoryURL = [[NSFileManager defaultManager] URLForDirectory:NSDocumentDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:NO error:nil];
...
- (void) checkIfFileIsPresent
{
    ...
    NSString *fiveMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Awakened Mind 5 - Beta.mp3"];
    NSString *fifteenMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Awakened Mind 15 - Beta.mp3"];
    NSString *thirtyMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Awakened Mind 30 - Beta.mp3"];
    
    fiveMinuteAwakenedMindExists = [[NSFileManager defaultManager] fileExistsAtPath:fiveMinuteMp3];
    
    fifteenMinuteAwakenedMindExists = [[NSFileManager defaultManager] fileExistsAtPath:fifteenMinuteMp3];
    
    thirtyMinuteAwakenedMindExists = [[NSFileManager defaultManager] fileExistsAtPath:thirtyMinuteMp3];
    
    if (fiveMinuteAwakenedMindExists == YES && fifteenMinuteAwakenedMindExists == YES && thirtyMinuteAwakenedMindExists == YES)
    {
...
//                                                                    [progress removeObserver:self forKeyPath:@"fractionCompleted" context:NULL];
                                                                    fiveMinutesDone = YES;
                                                                    NSLog(@"FiveMinute AwakenedMind Download Done");
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:nil destination:^NSURL *(NSURL *targetPath, NSURLResponse *response){
            ...
            return [documentsDirectoryURL URLByAppendingPathComponent:[response suggestedFilename]];
                                                                ...
                                                                completionHandler:^(NSURLResponse *response, NSURL *filePath, NSError *error){
                                                                    NSLog(@"FifteenMinute AwakenedMind Download Done");
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:&progress destination:^NSURL *(NSURL *targetPath, NSURLResponse *response){
            ...
            return [documentsDirectoryURL URLByAppendingPathComponent:[response suggestedFilename]];
                                                                ...
                                                                completionHandler:^(NSURLResponse *response, NSURL *filePath, NSError *error){
                                                                    NSLog(@"thirtyMinute AwakenedMind Download Done");
...
- (void)ifAllDoneRemoveDownloadWindow
{
    ...
    if (allMP3sDownloaded)
    {
        [userDefaults setValue:@"complete" forKey:@"awakenedMindDownloadStatus"];
        [userDefaults setBool:NO forKey:@"hasStartedDownloadingAwakendMind"];
        [userDefaults synchronize];
        buyPackView.hidden = YES;
        lockedButtonOutlet.hidden = YES;
        timeOptionsPickerView.hidden = NO;
