...
- (void)viewDidLoad
{
    ...
    zen180_IAP_Helper.creativeMindVC = self;
    ...
    {
        if ([[NSUserDefaults standardUserDefaults]boolForKey:@"hasStartedDownloadingCreativeMind"] == YES)
        {
            [self showUnlockButton];
            viewHasAlreadyLoadedOnce = YES;
        }
        else if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"creativeMindDownloadStatus"]  isEqual: @"complete"])
        {
...
{
    appDelegate.currentView = @"creativeMind";
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (fiveMinuteCreativeMindExists)
        [mp3Array addObject:@"1"];
    if (fifteenMinuteCreativeMindExists)
        [mp3Array addObject:@"1"];
    if (thirtyMinuteCreativeMindExists)
        [mp3Array addObject:@"1"];
    ...
    }else if (viewHasAlreadyLoadedOnce == NO)
    {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.CreativeMind_IAP"] == YES || [[NSUserDefaults standardUserDefaults] boolForKey:@"com.urbndna.Pack_IAP"] == YES)
        {
            ...
            else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"hasStartedDownloadingCreativeMind"] == YES)
            {
...
- (void)productPurchased:(NSNotification *)notification
{
    ...
    [productsArray enumerateObjectsUsingBlock:^(SKProduct *product, NSUInteger idx, BOOL *stop) {
        if ([product.productIdentifier isEqualToString:@"com.urbndna.CreativeMind_IAP"] || [product.productIdentifier isEqualToString:@"com.urbndna.Pack_IAP"])
        {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    {
        [self setupAudioPlayer:@"Creative Mind 5 - Theta"];
    ...
    {
        [self setupAudioPlayer:@"Creative Mind 15 - Theta"];
        [self.playButton setImage:[UIImage imageNamed:@"play-64.png"] forState:UIControlStateNormal];
    }
    
    else
        [self setupAudioPlayer:@"Creative Mind 30 - Theta"];
    [self.playButton setImage:[UIImage imageNamed:@"play-64.png"] forState:UIControlStateNormal];
...
- (IBAction)buySingleTrack:(id)sender
{
    ...
    {
        if ([product.productIdentifier isEqualToString:@"com.urbndna.CreativeMind_IAP"])
        {
    ...
    [[Zen180_IAP_Helper sharedInstance] buyProduct:product];
        ...
        [userDefaults setBool:YES forKey:@"hasStartedDownloadingCreativeMind"];
...
- (void)redownload
{
    if(!fiveMinuteCreativeMindExists && fifteenMinuteCreativeMindExists && thirtyMinuteCreativeMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fifteenMinuteDownload && fiveMinuteCreativeMindExists && thirtyMinuteCreativeMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!thirtyMinuteCreativeMindExists && fiveMinuteCreativeMindExists && fifteenMinuteCreativeMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fiveMinuteCreativeMindExists && !fifteenMinuteCreativeMindExists && thirtyMinuteCreativeMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fifteenMinuteCreativeMindExists && !thirtyMinuteCreativeMindExists && fiveMinuteCreativeMindExists)
    {
        buyPackView.hidden = NO;
    ...
    else if (!fiveMinuteCreativeMindExists && !thirtyMinuteCreativeMindExists && fifteenMinuteCreativeMindExists)
    {
        buyPackView.hidden = NO;
...
- (void)downloadFiveMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Creative%20Mind%205%20-%20Theta.mp3"];
...
- (void)downloadFifteenMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Creative%20Mind%2015%20-%20Theta.mp3"];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
...
- (void)downloadThirtyMinuteMp3
{
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSURL *url = [[NSURL alloc] initWithString:@"http://cf56da121053c8adb35e-38b175f4a644da114207134d2e8e6556.r62.cf1.rackcdn.com/Creative%20Mind%2030%20-%20Theta.mp3"];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        NSProgress *progress;
        NSURLSessionDownloadTask *downloadTask = [manager downloadTaskWithRequest:request progress:&progress destination:^NSURL *(NSURL *targetPath, NSURLResponse *response){
            NSURL *documentsDirectoryURL = [[NSFileManager defaultManager] URLForDirectory:NSDocumentDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:NO error:nil];
...
- (void)successAlert
{
    successAlert = [[UIAlertView alloc] initWithTitle:@"Message" message:@"Creative Mind- Download Successful" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
    [successAlert show];
    [userDefaults setValue:@"complete" forKey:@"creativeMindDownloadStatus"];
    [userDefaults synchronize];
    buyPackView.hidden = YES;
    lockedButtonOutlet.hidden = YES;
    timeOptionsPickerView.hidden = NO;
...
- (void) checkIfFileIsPresent
{
    ...
    NSString *fiveMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Creative Mind 5 - Theta.mp3"];
    NSString *fifteenMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Creative Mind 15 - Theta.mp3"];
    NSString *thirtyMinuteMp3 = [documentsDirectory stringByAppendingPathComponent:@"Creative Mind 30 - Theta.mp3"];
    
    fiveMinuteCreativeMindExists = [[NSFileManager defaultManager] fileExistsAtPath:fiveMinuteMp3];
    
    fifteenMinuteCreativeMindExists = [[NSFileManager defaultManager] fileExistsAtPath:fifteenMinuteMp3];
    
    thirtyMinuteCreativeMindExists = [[NSFileManager defaultManager] fileExistsAtPath:thirtyMinuteMp3];
    
    if (fiveMinuteCreativeMindExists == YES && fifteenMinuteCreativeMindExists == YES && thirtyMinuteCreativeMindExists == YES)
    {
...
- (void)ifAllDoneRemoveDownloadWindow
{
    ...
    {
        [userDefaults setValue:@"complete" forKey:@"creativeMindDownloadStatus"];
        [userDefaults setBool:NO forKey:@"hasStartedDownloadingCreativeMind"];
        [userDefaults synchronize];
        buyPackView.hidden = YES;
        lockedButtonOutlet.hidden = YES;
        timeOptionsPickerView.hidden = NO;
